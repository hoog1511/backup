...
namespace Domotica
{
    ...
    public class MainActivity : Activity
    {
        ...
        Button send_time_on;
        Button send_time_off;
        Switch ChangePinState1;
        Switch ChangePinState2;
        Switch switch_sensor_mode;
        TextView textViewServerConnect, textViewTimerStateValue;
        public TextView textViewChangePinStateValue, textViewSensorValue, textViewDebugValue, textViewChangePinStateValue2, time_on_text , time_off_text;
        EditText editTextIPAddress, editTextIPPort, lightson, lightsoff;
        ...
        {
            base.OnCreate(bundle); // gooit alle data erin in een groepje.
            ...
            send_time_on = FindViewById<Button>(Resource.Id.send_time_on);
            send_time_off = FindViewById<Button>(Resource.Id.send_time_off);
            ChangePinState1 = FindViewById<Switch>(Resource.Id.ChangePinState1);
            ChangePinState2 = FindViewById<Switch>(Resource.Id.ChangePinState2);
            switch_sensor_mode = FindViewById<Switch>(Resource.Id.switch_sensor_mode);
            ...
            time_on_text = FindViewById<TextView>(Resource.Id.time_on_text);
            time_off_text = FindViewById<TextView>(Resource.Id.time_off_text);
            editTextIPAddress = FindViewById<EditText>(Resource.Id.editTextIPAddress);
            editTextIPPort = FindViewById<EditText>(Resource.Id.editTextIPPort);
            lightson = FindViewById<EditText>(Resource.Id.lightson);
            lightsoff = FindViewById<EditText>(Resource.Id.lightsoff);
            ...
            //Add the "Changepinstate1" switch handler.
            if (ChangePinState1 != null)
            {
                ChangePinState1.CheckedChange += delegate (object sender, CompoundButton.CheckedChangeEventArgs e)
                {
            ...
            //Add the "Changepinstate2" switch handler.
            if (ChangePinState2 != null)
            {
                ChangePinState2.CheckedChange += delegate (object sender, CompoundButton.CheckedChangeEventArgs e)
                {
                    socket.Send(Encoding.ASCII.GetBytes("u")); 
                };
            }

            //Add the "switch_sensor_mode" switch handler.
            if (switch_sensor_mode != null)
            {
                switch_sensor_mode.CheckedChange += delegate (object sender, CompoundButton.CheckedChangeEventArgs e)
                {
                    socket.Send(Encoding.ASCII.GetBytes("d"));
                };
            }

            //ADD the "send_time_on" button handler.
            if (send_time_on != null)
            {
                send_time_on.Click += (sender, e) =>
                {
                    socket.Send(Encoding.ASCII.GetBytes("i"));
                    socket.Send(Encoding.ASCII.GetBytes(lightson.Text));
                    time_on_text.Text = lightson.Text;
                };
            }

            //ADD the "send_time_off" button handler.
            if (send_time_off != null)
            {
                send_time_off.Click += (sender, e) =>
                {
                    socket.Send(Encoding.ASCII.GetBytes("o"));
                    socket.Send(Encoding.ASCII.GetBytes(lightsoff.Text));
                    time_off_text.Text = lightsoff.Text;
                };
            }
        ...
        public void UpdateConnectionState(int state, string text)
        {
            ...
            RunOnUiThread(() =>
            {
                ...
                ChangePinState1.Enabled = butPinEnabled;
                ChangePinState2.Enabled = butPinEnabled;
                switch_sensor_mode.Enabled = butPinEnabled;
                send_time_on.Enabled = butPinEnabled;
                send_time_off.Enabled = butPinEnabled;
            });
        }
